...
I <- which(Y > 2)
Ie <- sample(I, size = 1e3, replace = FALSE)
Xe <- X[Ie,]
Ye <- Y[Ie]
...
log.f.real <- function(b) {
  s <- 0
  for (j in 1:length(Y)) {
    s <- s + as.numeric(Y[j]*t(Dx[j,]) %*% b - (r + Y[j]) * log(r + exp(t(Dx[j,]) %*% b)))
  }
  return(s)
}
...
proposal <- function(X, Y) {
  X_new <- cbind(1, X)
  beta.hat <- glm.nb(Y ~ X)$coefficients
...
proposal_new_approx <- function(X, Y, k = 5) {
  ...
  De <- numeric(length(colnames(X_new)))
  for (i in 1:length(X_new[, 1])) {
    H <- H - r*(r + Y[i])*exp(as.numeric((X_new[i,]) %*% beta.hat))/(r + exp(as.numeric((X_new[i, ]) %*% beta.hat)))^2 * (X_new[i, ]) %*% t(X_new[i, ])
    De <- De + (-Y[i] * X_new[i,] + (r + Y[i]) * exp(beta.hat %*% X_new[i, ])/(r + exp(beta.hat %*% X_new[i,])) * X_new[i,])
  }
...
proposal_new_real <- function(X, Y, Xr, Yr) {
  ...
  for (i in 1:length(X_r_new[,1])) {
    H_real <- H_real + r*(r + Yr[i])*exp(as.numeric((X_r_new[i,]) %*% beta.hat))/(r + exp(as.numeric((X_r_new[i, ]) %*% beta.hat)))^2 * (X_r_new[i, ]) %*% t(X_r_new[i, ])
  }
  L <- list()
  L[["H Approx"]] <- H
  L[["H"]] <- H_real
...
log.f <- function(X, Y, beta) {
  ...
  for (i in 1:length(Y)) {
    ...
    s <- s + Y[i] * (X[i, ] %*% beta) - (r + Y[i])*log(r + exp(X[i, ] %*% beta))
  }
  #  print(s)
  return(s)
}
...
mcmc_nuevo_approx <- function(Xr, Yr, Xe, Ye, n = 1e3, h = 1e-6, k = 5) {
  XE <- cbind(1, Xe)
  XR <- cbind(1, Xr)
  X <- rbind(Xe, Xr)
  Y <- append(Ye, Yr)
  XB <- cbind(1, X)
  beta.not <- glm.nb(Y ~ X)$coefficients
  # beta.not <- rmvnorm(1, mean = beta.not, sigma = 1e-3 * diag(1, 3, 3))
...
mcmc_mala <- function(Xr, Yr, Xe, Ye, n = 1e3, h = 1e-6, k = 5) {
  XE <- cbind(1, Xe)
  XR <- cbind(1, Xr)
  X <- rbind(Xe, Xr)
  Y <- append(Ye, Yr)
...
drawSamplesReal <- function(n, h = 1) {
  M <- matrix(0, 3, n)
  a <- 0
  Dr <- cbind(1, Xr)
#  M[,1] <- rmvnorm(1, c(0, 0, 0), sigma = h*diag(1, 3, 3))
  M[, 1] <- glm.nb(Y ~ X)$coefficients
  for (i in 2:n) {
    print(i)
    y <- M[,(i - 1)] + rmvnorm(1, c(0, 0, 0), sigma = h*diag(1, 3, 3))
...
#5e-5
ta <- system.time(maF <- mcmc_nuevo_approx(Xr, Yr, Xe, Ye, n = 1e3, h = 5e-4, k = 150))
print("Approx")
# mnF <- mcmc_nuevo(Xr, Yr, Xe, Ye, n = 5e2, h = 5e-5)
# print("Unlearning")
# 5e-7
tm <- system.time(mmF <- mcmc_mala(Xr, Yr, Xe, Ye, n = 1e3, h = 1e-7))
print("MALA")
tr <- system.time(mrF <- drawSamplesReal(1e3, h = 0.000025))
print("Real")
ma <- maF$M
#mn <- mnF$M
mm <- mmF$M
...
#b <- benchmark(mcmc_nuevo_approx(Xr, Yr, Xe, Ye, n = 1e2, h = 5e-4, k = 150), mcmc_mala(Xr, Yr, Xe, Ye, n = 1e2, h = 1e-7), drawSamplesReal(1e2, h = 0.000025), replications = 1)
#b
#kable(b, "latex")

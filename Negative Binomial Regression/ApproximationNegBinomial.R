...
drawSamples <- function(n, h = 1) {
  ...
  for (j in 1:length(Y)) {
    H.samples[[toString(M[,1])]] <- H.samples[[toString(as.vector(M[,1]))]] + r * (r + Y[j]) * as.numeric(exp(t(Dx[j,]) %*% as.vector(M[,1]))/((r + exp(t(Dx[j,])%*%as.vector(M[,1])))^2)) * (Dx[j,] %*% t(Dx[j,]))
  }
  for (i in 2:n) {
    print(i)
    ...
    if (log(runif(1)) < log.alpha) {
      ...
      M[,i] <- y
      for (j in 1:length(Y)) {
        H.samples[[toString(as.vector(y))]] <- H.samples[[toString(as.vector(y))]] + r * (r + Y[j]) * as.numeric(exp(t(Dx[j,]) %*% as.vector(y))/((r + exp(t(Dx[j,])%*%as.vector(y)))^2)) * (Dx[j,] %*% t(Dx[j,]))
      }
    }
    else {
      M[,i] <- M[,(i - 1)]
...
for (i in (n + 1):(2*n)) {
  ...
  for (j in 1:length(Y)) {
    log.F.samples[[toString(beta[,i])]] <- log.F.samples[[toString(beta[,i])]] + as.numeric(Y[j]*t(Dx[j,]) %*% beta[,i] - (r + Y[j])*log(r + exp(t(Dx[j,]) %*% beta[,i])))
  }
}
...
for (i in (n + 1):(2*n)) {
  ...
  for (j in 1:length(Y)) {
    H.samples[[toString(beta[,i])]] <- H.samples[[toString(beta[,i])]] + r * (r + Y[j]) * as.numeric(exp(t(Dx[j,]) %*% beta[,i])/((r + exp(t(Dx[j,])%*%beta[,i]))^2)) * (Dx[j,] %*% t(Dx[j,]))
  }
}
...
be <- c(-0.5, -1.2, -0.8)
log.f.real(be)
as.numeric(approx(be, k = 5)$log.f)

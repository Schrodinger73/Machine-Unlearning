...
proposal <- function(X, Y) {
  X_new <- cbind(1, X)
  beta.hat <- glm(Y ~ X, family = poisson)$coefficients
  ...
  for (i in 1:length(X_new[, 1])) {
    H <- H - as.numeric(exp(t(X_new[i,]) %*% as.vector(beta.hat))) * X_new[i,] %*% t(X_new[i,])
    De <- De + (exp(as.numeric((X_new[i,]) %*% beta.hat)) - Y[i]) * X_new[i, ]
  }
  L <- list()
  L[["H"]] <- H
  L[["D"]] <- De
...
proposal_new_real <- function(X, Y, Xr, Yr) {
  ...
  for (i in 1:length(X_r_new[,1])) {
    H_real <- H_real + as.numeric(exp(t(X_r_new[i,]) %*% as.vector(beta.hat))) * X_r_new[i,] %*% t(X_r_new[i,])
  }
  L <- list()
  L[["H Approx"]] <- H
  L[["H"]] <- H_real
...
log.f <- function(X, Y, beta) {
  ...
  for (j in 1:length(Y)) {
    s <- s + as.numeric(Y[j]*t(X[j,]) %*% beta - exp(t(X[j,]) %*% beta))
  }
  return(s)
}
...
mcmc_nuevo_approx <- function(Xr, Yr, Xe, Ye, n = 1e3, h = 1e-6, k = 5) {
  ...
  Y <- append(Ye, Yr)
  XB <- cbind(1, X)
  beta.not <- glm(Y ~ X, family = poisson)$coefficients
...
#5e-6
ta <- system.time(maF <- mcmc_nuevo_approx(Xr, Yr, Xe, Ye, n = 1e3, h = 5e-8, k = 150))
print("Approx")

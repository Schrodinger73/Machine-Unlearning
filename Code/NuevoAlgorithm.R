...
mcmc_nuevo <- function(Xr, Yr, Xe, Ye, n = 1e3, h = 1e-6) {
  ...
  XB <- cbind(1, X)
  beta.not <- glm(Y ~ X, family = binomial)$coefficients
  beta.not <- rmvnorm(1, mean = beta.not, sigma = diag(1, 3, 3))
  M <- matrix(0, length(XE[1,]), n)
  M[,1] <- beta.not
  #  prop <- proposal(Xe, Ye)
...
#5e-6
maF <- mcmc_nuevo_approx(Xr, Yr, Xe, Ye, n = 1e3, h = 1e-1, k = 150)
print("Approx")
mrF <- drawSamplesReal(1e3, h = 5e-4)
print("Real")
ma <- maF$M
mr <- mrF$M
rowMeans(ma)
...
benchmark(mcmc_nuevo_approx(Xr, Yr, Xe, Ye, n = 1e3, h = 1e-1, k = 150), drawSamplesReal(1e3, h = 5e-4), replications = 1)
